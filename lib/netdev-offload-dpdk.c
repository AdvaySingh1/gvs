...
#define GIGAFLOW_OFFLOAD_LOW_PRIORITY 0xff
#define GIGAFLOW_OFFLOAD_STANDARD_PRIORITY 0x64
...
#define DUMP_PATTERN_ITEM(mask, has_last, field, fmt, spec_pri, mask_pri,                                                     \
    ...
    if (has_last)                                                                                                             \
    {                                                                                                                         \
        ds_put_format(s, field " spec " fmt " " field " mask " fmt " " field " last " fmt " ", spec_pri, mask_pri, last_pri); \
    }                                                                                                                         \
                ...
                struct netdev *tnldev)
{
    ...
    if (ret)
    {
        VLOG_DBG_RL(&rl, "%s: netdev_dpdk_rte_flow_tunnel_match failed: "
                         "%d (%s).",
                    netdev_get_name(physdev), error.type,
                    error.message);
        goto out;
    }
                   ...
                   uint8_t proto)
{
                           ...
                           sizeof(struct in6_addr)))
    {
        ...
        spec->hdr.vtc_flow = htonl((uint32_t)match->flow.tunnel.ip_tos << RTE_IPV6_HDR_TC_SHIFT);
        ...
        mask->hdr.vtc_flow = htonl((uint32_t)match->wc.masks.tunnel.ip_tos << RTE_IPV6_HDR_TC_SHIFT);
                 ...
                 struct match *match)
{
    ...
    if (match->flow.dl_type == htons(ETH_TYPE_IP))
    {
        ...
        if (match->wc.masks.nw_frag & FLOW_NW_FRAG_ANY)
        {
            if (!(match->flow.nw_frag & FLOW_NW_FRAG_ANY))
            {
                ...
                mask->hdr.fragment_offset = htons(RTE_IPV4_HDR_OFFSET_MASK | RTE_IPV4_HDR_MF_FLAG);
            }
    ...
    if (match->flow.dl_type == htons(ETH_TYPE_IPV6))
    {
        ...
        memcpy(spec->hdr.dst_addr, &match->flow.ipv6_dst,
               sizeof spec->hdr.dst_addr);
        if ((match->wc.masks.nw_frag & FLOW_NW_FRAG_ANY) && (match->flow.nw_frag & FLOW_NW_FRAG_ANY))
        {
            spec->has_frag_ext = 1;
        }
...
netdev_offload_dpdk_flow_destroy(struct ufid_to_rte_flow_data *rte_flow_data)
{
    ...
    if (ret == 0)
    {
        ...
        VLOG_DBG_RL(&rl, "%s/%s: rte_flow 0x%" PRIxPTR " flow destroy %d ufid " UUID_FMT,
...
ovs_flow_to_p4sdnet_key(struct match *match, uint8_t *entry_key)
{
    ...
    // TODO ask about this
    entry_key[P4SDNET_KEY_B0_TABLE_TAG] = match->flow.nw_tos;
...
ovs_mask_to_p4sdnet_mask(struct match *match, uint8_t *entry_mask)
{
    ...
    entry_mask[P4SDNET_KEY_B24_TOS_B0] = 0x00;
...
{
    /*
     * Netlink attr header: 4 bytes
     * Nested attr header: 4 bytes
     * ipv4_src: 4 bytes
     * ipv4_dst: 4 bytes
     * ipv4_proto: 1 byte
     * ipv4_tos: 1 byte <- target at offset 17
     */
    uint8_t table_tag = *((uint8_t *)table_tag_nla + 17);
    return table_tag;
}
...
ovs_priority_to_p4sdnet_priority(uint32_t ovs_priority)
{
    ...
    // }
    // else if (ovs_priority >= GIGAFLOW_STANDARD_PRIORITY)
    // {
    //     return GIGAFLOW_OFFLOAD_STANDARD_PRIORITY;
    // }
    // else
    // {
    //     return GIGAFLOW_OFFLOAD_DEFAULT_PRIORITY;
    // }
}
                                ...
                                struct dpif_flow_stats *stats)
{
    ...
    uint32_t entry_priority = ovs_priority_to_p4sdnet_priority(match->gf_priority);
    uint32_t entry_action_id;
    uint8_t entry_table_id;
...
static int
netdev_offload_p4sdnet_flow_get(struct netdev *netdev, struct match *match,
                                struct nlattr *actions, size_t actions_len,
                                const ovs_u128 *ufid, struct offload_info *info,
                                struct dpif_flow_stats *stats)
{
...
static int
netdev_offload_p4sdnet_flow_flush(struct netdev *netdev)
{
    return 0;
}

static int
netdev_offload_p4sdnet_hw_miss_packet_recover(struct netdev *netdev,
                                              struct dp_packet *packet)
{
    return 0;
}
...
static int
netdev_offload_p4sdnet_get_n_flows(struct netdev *netdev,
                                   uint64_t *n_flows)
{
    return 0;
}
